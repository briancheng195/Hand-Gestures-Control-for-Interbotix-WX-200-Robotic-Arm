...
def main():
    ...
    def sensor_data_callback(msg):
        ...
        pitch_mapped = map_value(msg.data[0], -90, 90, np.pi/2, -np.pi/2)
        print(f'pitch movement: {pitch_mapped}')

        yaw_mapped = map_value(msg.data[1], -90, 90, np.pi/2, -np.pi/2)
        print(f'yaw movement: {yaw_mapped}')

        roll_mapped = map_value(msg.data[2], -90, 90, np.pi/2, -np.pi/2)
        print(f'roll movement: {roll_movement}')
        
        linear_mapped = map_value(msg.data[3], -5, 5, 248.32, 500)
        ...
        bot.arm.set_single_joint_position(joint_name = 'wrist_rotate', position=roll_mapped)
